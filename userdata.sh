#!/bin/sh
# AUTO-GENERATED by scripts/make-userdata.sh from bootstrap.sh.
# DO NOT EDIT — edit bootstrap.sh and re-run the generator.
#
# Dash-safe wrapper for Lightsail user-data. Lightsail prepends its
# own #!/bin/sh SSH-CA setup, so this whole file must parse in dash.
# We dump bootstrap.sh to disk via a quoted heredoc (no expansion)
# and exec bash on the result.

cat > /root/jinx-bootstrap.sh <<'BOOTSTRAP_PAYLOAD_END_MARKER'
#!/usr/bin/env bash
...
BOOTSTRAP_PAYLOAD_END_MARKER
chmod +x /root/jinx-bootstrap.sh
exec bash /root/jinx-bootstrap.sh
